...
var addExtraMethods = function(Account) {
  ...
  // Recover Account
  Account.beforeRemote('RecoverAccount', function(ctx, instance, next) {
    // Validate
    let tokenParam = ctx.req.params.token;
    let body = ctx.req.body;
    
    let requiredKeys = !tokenParam ? ['email'] : ['password'];
    body = _.pick(ctx.req.body, requiredKeys)
    
    if (_.keys(body).length === requiredKeys.length) {
      ctx.req.body = body;
      next();
    } else {
      next({
        status: 400,
        message: `${requiredKeys.join(' and ')} field is required.`
      });
    }
  });

  Account.afterRemote('RecoverAccount', function (ctx, instance, next) {
      ctx.result = {
        status: "ok" 
      }
      next();
  ...
  Account.RecoverAccount = function(ctx, cb) {
    let tokenParam = ctx.req.params.token;
    let body = ctx.req.body;
    if (!tokenParam) {
        async.waterfall([
          ...
          (account, callback) => {
            if (!account) return callback(null, { status: 200, message: 'email sent' });
            let salt = crypto.randomBytes(8).toString('hex');
            let token = crypto
              .createHmac('md5', `${salt}${Date.now()}`)
          ...
          (token, callback) => {
            if (!token) return callback(null, { status: 200, message: 'email sent' });
            // SEND WELCOME EMAIL
            Account.app.models.Mailer.send({
              to: body.email,
              from: 'no-reply@narengi.com',
        ...
        async.waterfall([
            ...
            (token, callback) => {
                if (!token) return callback({ status: 404, message: "token is invalid" });
                token.updateAttributes({ is_active: false });
                Account.findOne({
                    where: {
                        id: token.assigned_account
                    }
                }, callback)
            },
            (account, callback) => {
